...
export class CertificatesService {
  ...
  async generatePdf(id: string, _userId: string): Promise<Buffer> {
    const [cert] = await this.drizzle.db
      .select({
        id: courseCertificates.id,
        issuedAt: courseCertificates.issuedAt,
    ...
    const navyBlue = rgb(0.051, 0.106, 0.243);
    ...
    page.drawRectangle({ x: 20, y: 20, width: width - 40, height: 8, color: navyBlue });
    page.drawRectangle({ x: 20, y: height - 28, width: width - 40, height: 8, color: navyBlue });
    ...
    page.drawText(cert.holderName ?? 'Candidate', {
      x: width / 2 - (cert.holderName?.length ?? 10) * 11,
      y: height - 260,
      size: 32,
      font: timesBold,
      color: navyBlue,
    ...
    page.drawLine({
      start: { x: 150, y: height - 272 },
      end: { x: width - 150, y: height - 272 },
      thickness: 1,
      color: gold,
    });
    ...
    const titleLines = this.wrapText(cert.courseTitle ?? '', 50);
    titleLines.forEach((line, i) => {
      page.drawText(line, {
        x: width / 2 - (line.length * 9),
        y: height - 345 - i * 30,
        size: 22,
        font: timesBold,
        color: navyBlue,
    ...
    page.drawText(`${cert.cpdHours} CPD Hours`, {
      x: width / 2 - 55,
      y: cpdY + 4,
      size: 14,
      font: timesBold,
    ...
    page.drawText('Verify at bukzaccounting.co.uk/verify', {
      x: width - 250,
      y: 52,
      size: 9,
      font: helvetica,
